...
interface GameContentHelpModalProps {
    gameId: GameTypeId;
    isOpen: boolean;
    onClose: () => void;
    translations: any;
...
export default function GameContentHelpModal({
    gameId,
    isOpen,
    onClose,
    translations
}: GameContentHelpModalProps) {
    if (!isOpen) return null;

    const helpContent = translations.gamification.contentHelp[gameId];
        ...
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 animate-in fade-in duration-200">
            ...
            <div className="relative bg-white dark:bg-slate-900 rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden border-2 border-indigo-100 dark:border-indigo-900/30 animate-in zoom-in-95 duration-300">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-100 text-sm font-medium">
                                {translations.gamification.contentHelp.title}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
                    ...
                    <div className="mb-6">
                        <h3 className="text-lg font-black text-slate-800 dark:text-white mb-4 flex items-center gap-2">
                            ...
                            {helpContent.whatToCreate}
                        </h3>
                        <ul className="space-y-3">
                            {helpContent.items.map((item: string, index: number) => (
                                <li
                                    key={index}
                ...
                <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50">
                    ...
                    >
                        {translations.gamification.contentHelp.close}
                    </button>
                </div>
            </div>
        </div>
